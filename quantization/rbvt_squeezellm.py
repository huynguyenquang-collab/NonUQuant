...
def stats_are_finite(means, variances):
    bad = []
    for group_name, group in (("means", means), ("variances", variances)):
        for name, tensor in group.items():
            if not torch.isfinite(tensor).all():
                bad_count = int((~torch.isfinite(tensor)).sum().item())
                bad.append(f"{group_name}:{name}({bad_count})")
                if len(bad) >= 8:
                    return False, bad
    return True, bad
...
def build_module_map(model, model_type):
    ...
    for layer_idx, layer in enumerate(layers):
        for short_name, module in get_named_modules(layer, model_type):
...
def collect_activation_stats(args):
    if args.stats_path and os.path.exists(args.stats_path) and not args.overwrite_stats:
        ...
        means, variances = payload["means"], payload["variances"]
        finite, bad = stats_are_finite(means, variances)
        if finite:
            return means, variances
        print(
            "Existing RBVT activation stats contain non-finite values; recollecting. "
            f"First bad entries: {', '.join(bad)}",
            flush=True,
        )
    ...
    if not finite:
        raise RuntimeError(
            "RBVT activation stats contain non-finite values after collection. "
            "Use --model_dtype bfloat16 for Qwen/Gemma-style models. "
            f"First bad entries: {', '.join(bad)}"
        )
    if args.stats_path:
...
def sort_centers_and_remap_labels(centers, labels):
    order = torch.argsort(centers, dim=1)
    sorted_centers = torch.gather(centers, 1, order)
    inverse = torch.empty_like(order)
    inverse.scatter_(1, order, torch.arange(order.shape[1], device=order.device).expand_as(order))
    sorted_labels = torch.gather(inverse, 1, labels.long())
...
def apply_rbvt_indices(W_fp, labels, centers, mu, sigma_ii, args):
    if args.rbvt_lambda < 0.0:
        raise ValueError(f"rbvt_lambda must be non-negative, got {args.rbvt_lambda}")
    if not 0.0 <= args.rbvt_budget_p <= 1.0:
        raise ValueError(f"rbvt_budget_p must be in [0, 1], got {args.rbvt_budget_p}")
    if not 0.0 <= args.rbvt_target_ratio <= 1.0:
        raise ValueError(f"rbvt_target_ratio must be in [0, 1], got {args.rbvt_target_ratio}")

    device = W_fp.device
    out_features, _ = W_fp.shape
    levels, idx_full = sort_centers_and_remap_labels(
        centers.to(device).float(),
        labels.to(device).long(),
    )
    idx_full = idx_full.clone()
    ...
    for r0 in range(0, out_features, args.row_chunk):
        ...
        for rr in range(rc):
            ...
            if args.rbvt_budget_p <= 0.0:
                stats.bias_after += base_obj
                stats.objective_after += base_obj
                continue
            if args.rbvt_budget_p < 1.0:
                cand = cand[: max(1, math.ceil(args.rbvt_budget_p * int(cand.numel())))]
            if args.rbvt_topk > 0 and cand.numel() > args.rbvt_topk:
                cand = cand[: args.rbvt_topk]

            r_cand = r[rr, cand]
            q_cand = q[rr, cand]
            limit = T_eff if not args.allow_overshoot else 2.0 * T_eff
            ...
            gamma_star = (T_eff - s_prev - args.rbvt_lambda * q_cand / (2.0 * (r_cand + eps))) / (
                r_cand + eps
            )
            gamma = torch.minimum(torch.maximum(gamma_star, torch.zeros_like(gamma_star)), upper)
            relaxed = (T_eff - s_prev - gamma * r_cand).square() + args.rbvt_lambda * (
                q_prev + gamma * q_cand
            )
            ...
            drop_obj = (T_eff - prefix_r) ** 2 + args.rbvt_lambda * prefix_q
            ...
            if float(gamma[best_pos_i].item()) > 0.0:
                ...
                if keep_r <= limit + 1e-8:
                    ...
                    keep_obj = (T_eff - keep_r) ** 2 + args.rbvt_lambda * keep_q
    ...
    return idx_full.cpu().numpy().astype("uint8"), levels.cpu().numpy(), stats
...
def apply_rbvt(args):
    ...
    for layer_idx in tqdm(layer_ids, desc="Applying RBVT-Squeeze"):
        ...
        out_layer = {}
        for module_name in model_layer.keys():
            ...
            new_labels, centers_np, stats = apply_rbvt_indices(
...
def main():
    ...
    parser.add_argument(
        ...
        help="Model dtype for RBVT activation stats. Use bfloat16 for Qwen/Gemma-style models to avoid fp16 overflow.",
    )
    ...
    parser.add_argument("--rbvt_budget_p", type=float, default=1.0)
    parser.add_argument("--rbvt_target_ratio", type=float, default=1.0)
    parser.add_argument("--rbvt_mse_guard", action="store_true")
