...
def llama_sequential(model, folder, include_sparse, model_type=None):
    ...
    for i in range(len(layers)):
        ...
        sequential_lut = get_named_sequential(layers[i], model_type)
        outlier_index = {name: idx for idx, (name, _path) in enumerate(sequential_lut)}

        for s, real_name in sequential_lut:
            lut = lut_layer[s]
            if include_sparse:
                idx = outlier_index[s]
                ...
                outliers = None
            quantizers[f"{layers_name}.{i}.{real_name}"] = [lut, outliers]
