...
from squeezellm.model_parse import parse_model
...
def sanitize_sample_weight(sample_weight, weight_mask):
    sample_weight = np.asarray(sample_weight, dtype=np.float64)
    weight_mask = np.asarray(weight_mask, dtype=bool)
    invalid = ~np.isfinite(sample_weight) | (sample_weight < 0)
    if invalid.any():
        sample_weight = sample_weight.copy()
        sample_weight[invalid] = 0.0

    total = sample_weight.sum(dtype=np.float64)
    if not np.isfinite(total) or total <= 0:
        sample_weight = weight_mask.astype(np.float64)
        if sample_weight.sum(dtype=np.float64) <= 0:
    ...
    for l in ran:
        ...
        for name in module_names:
            ...
            for i in range(module_weight.shape[0]):
                ...
                sample_weight = sanitize_sample_weight(g[i, :] * weight_mask, weight_mask)
